...
export default function MyApp({ Component, pageProps }:AppProps){
  ...
  useEffect(()=>{
    ...
    if(!document.getElementById(id)){
      const l=document.createElement('link'); l.id=id; l.rel='stylesheet'; l.href='/styles/late.css'; document.head.appendChild(l);
    }
  },[]);
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if(c) setAnalyticsConsent(c.endsWith('yes'));
    } catch {}
  },[]);
  return <>
...
function inferPageTypeFromPath(pathname:string){
  if(/\/booking/.test(pathname)) return 'booking';
  if(/\/seo\//.test(pathname)) return 'seo';
  if(/\/fare$/.test(pathname)) return 'fare';
  return 'other';
}

function getSlug(kind:'origin'|'destination'){
  try {
    const parts = window.location.pathname.split('/').filter(Boolean);
    if(parts.length>=2){
      if(kind==='origin') return parts[0];
      if(kind==='destination') return parts[1];
    }
  }catch{}
  return undefined;
}
