...
interface RoutesIndexProps { routes:{ id:number; origin:string; destination:string; distance:number }[] }
export const getStaticProps: GetStaticProps<RoutesIndexProps> = async ()=>{
  const routes=await fetchRoutes();
  await disconnect();
  return { props:{ routes: routes.map((r:any)=>({ id:r.id, origin:r.origin.slug, destination:r.destination.slug, distance:r.distance_km })) }, revalidate };
};
