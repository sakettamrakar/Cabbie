...
export const getStaticProps = async ({ params, preview }) => {
    ...
    await disconnect();
    return { props: { origin, destination, distance: route.distance_km || 0, duration: route.duration_min || 0, fares, highlights: (hl === null || hl === void 0 ? void 0 : hl.highlights) || [], faqs: (faqs === null || faqs === void 0 ? void 0 : faqs.faqs) || [], related, routeId: route.id, updatedOn: updatedDate.toISOString() }, revalidate };
};
...
export default function FarePage({ origin, destination, distance, duration, fares, highlights, faqs, related, routeId, inactive, updatedOn }) {
    ...
    useEffect(() => {
        if (fares[0])
            track('quote_viewed', { origin, destination, car_type: fares[0].car_type, fare: fares[0].base });
        ...
        async function refresh() {
            ...
            try {
                ...
                if (j && Array.isArray(j.fares)) {
                    const mapped = j.fares.map((f) => ({ car_type: f.car_type, base: f.base_fare_inr || f.base }));
                    if (mapped.length) {
                        setLiveFares(mapped);
                        setFareError(undefined);
                    }
                }
            }
            catch (e) {
  ...
  <p style={{ fontSize: 12, marginTop: 4, color: '#555' }}>{translate('updated_on')} {new Date(updatedOn).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
      ...
      <section aria-labelledby="fareCardsHeading">
        ...
        <div className="fares-wrap" aria-busy={loadingFares ? 'true' : undefined}>
          {loadingFares && liveFares.length === 0 && Array.from({ length: 3 }).map((_, i) => (<div key={i} style={{ border: '1px solid #e2e8f0', padding: 12, borderRadius: 6, width: 200 }}>
      ...
      <section id="booking" style={{ marginTop: 32 }} aria-labelledby="bookingHeading">
        ...
        {bookingVisible && <BookingIsland routeId={routeId} preselectedCarType={selectedCar} defaultOrigin={origin} defaultDestination={destination} onBooked={(id) => { window.location.assign(`/booking/${id}`); }}/>}
        <noscript>
  ...
  {(related === null || related === void 0 ? void 0 : related.length) > 0 ? <ul>{related.map((r, i) => <li key={i}><a href={`/${r.origin}/${r.destination}/fare`}>{r.origin} → {r.destination}</a></li>)}</ul> :
            <div>
