export default async function handler(req, res) {
    ...
    try {
        ...
        if (data.status !== 'OK') {
            ...
            const fallback = calculateFallbackDistance(origin, destination);
...
function calculateFallbackDistance(origin, destination) {
    ...
    const normalizeCity = (city) => city.toLowerCase()
        .trim()
        .replace(/\s+/g, '-')
        .replace(/[^\w-]/g, '');
    ...
    if (cityDistances[route1]) {
        ...
        const getRegion = (city) => {
            ...
            if (northCities.some(c => lowerCity.includes(c)))
                return 'north';
            if (southCities.some(c => lowerCity.includes(c)))
                return 'south';
            if (westCities.some(c => lowerCity.includes(c)))
                return 'west';
            if (eastCities.some(c => lowerCity.includes(c)))
                return 'east';
            return 'unknown';
        };
