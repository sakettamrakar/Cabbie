...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(!booking_id || !status) return res.status(400).json({ ok:false, error:'booking_id & status required' });
  ...
  try {
    const updated = await prisma.booking.update({ where:{ id:Number(booking_id) }, data:{ status } as any });
