...
export default async function handler(req, res) {
    ...
    if (!booking_id || !driver_id)
        return res.status(400).json({ ok: false, error: 'booking_id & driver_id required' });
    try {
        ...
        if (driver.car_type !== booking.car_type)
            return res.status(400).json({ ok: false, error: 'Driver car type mismatch' });
