...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(req.method === 'POST'){
    ...
    if(!origin_city_id || !destination_city_id) return res.status(400).json({ ok:false, error:'Missing origin/destination' });
    const dupe = await prisma.route.findFirst({ where:{ origin_city_id, destination_city_id } });
    if(dupe) return res.status(409).json({ ok:false, error:'Route exists' });
