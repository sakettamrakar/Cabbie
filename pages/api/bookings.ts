...
import { generateEventId, hasEventId, rememberEventId } from '../../lib/analytics/dedupe';
...
export default withApi(async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(!idemKey || typeof idemKey !== 'string') return sendError(res, makeError('VALIDATION_FAILED',400,'Idempotency-Key header required'));
  ...
  let utm_source: string|undefined; let utm_medium: string|undefined; let utm_campaign: string|undefined;
  try {
    if(typeof (req as any).cookies === 'object'){
      const raw = (req as any).cookies['utm_first'];
      if(raw){ const parsed = JSON.parse(decodeURIComponent(raw)); utm_source = parsed.source; utm_medium = parsed.medium; utm_campaign = parsed.campaign; }
    } else if(req.headers.cookie){
      const hit = req.headers.cookie.split('; ').find(c=>c.startsWith('utm_first='));
      if(hit){ const val = hit.split('=')[1]; const parsed = JSON.parse(decodeURIComponent(val)); utm_source = parsed.source; utm_medium = parsed.medium; utm_campaign = parsed.campaign; }
    }
  } catch {}
  ...
  if(payment_mode && payment_mode !== 'COD') return sendError(res, makeError('VALIDATION_FAILED',400,'Only COD supported currently'));
  if(!route_id || typeof route_id !== 'number') return sendError(res, makeError('VALIDATION_FAILED',400,'route_id required'));
  if(!customer_phone || !/^[0-9]{10}$/.test(customer_phone)) return sendError(res, makeError('VALIDATION_FAILED',400,'customer_phone invalid'));
  if(typeof fare_quote_inr !== 'number') return sendError(res, makeError('VALIDATION_FAILED',400,'fare_quote_inr required as number'));
  if(!otp_token) return sendError(res, makeError('VALIDATION_FAILED',400,'otp_token required'));
  ...
  const response = await getOrSetIdempotent('booking:'+idemKey, async () => {
    ...
    if(!consumed.ok){
      if(consumed.reason==='used') throw makeError('ALREADY_USED',410,'OTP token already used');
      if(consumed.reason==='expired') throw makeError('UNAUTHORIZED',401,'OTP token expired');
    ...
    if(!route || !route.is_active) throw makeError('NOT_FOUND',404,'Route inactive or missing');
    ...
    if(quote.route_id !== route_id) throw makeError('NOT_FOUND',404,'Route mismatch');
  ...
  const booking = await prisma.booking.create({ data:{
      ...
      customer_phone,
      discount_code: quote.applied_discount?.code || null,
