...
export default withApi(async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(!route_id) return sendError(res, makeError('VALIDATION_FAILED',400,'route_id required'));
  ...
  res.end(`<html><body><h1>Booking Received</h1><p>ID: ${(booking as any).id}</p><p>Status: pending verification.</p><p><a href="/">Return home</a></p></body></html>`);
});
