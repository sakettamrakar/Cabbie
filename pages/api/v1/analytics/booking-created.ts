...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  try {
    ...
    if(!booking_id) return sendError(res, makeError('VALIDATION_FAILED',400,'booking_id required'));
    ...
    if(event_id) rememberEventId(event_id);
    res.json({ ok:true });
  } catch(e:any){ sendError(res, makeError('UNKNOWN',500,e.message||'Server error')); }
}