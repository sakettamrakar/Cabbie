// TODO: /api/v2 for route shape changes.
...
export default withApi(async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(req.method==='POST'){
    ...
    if(!origin_city_id || !destination_city_id) return sendError(res, makeError('VALIDATION_FAILED',400,'Missing origin/destination'));
    const dupe = await prisma.route.findFirst({ where:{ origin_city_id, destination_city_id } });
    if(dupe) return sendError(res, makeError('VALIDATION_FAILED',409,'Route exists'));
    const route = await prisma.route.create({ data:{ origin_city_id, destination_city_id, distance_km: distance_km??null, duration_min: duration_min??null, is_active } });
