...
export default withApi(async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(req.method==='GET'){
    ...
    if(!route) return sendError(res, makeError('NOT_FOUND',404,'Not found'));
    return jsonOk(res,{ route });
  }
  if(req.method==='PUT'){
    if(!requireCsrf(req,res)) return; const { distance_km, duration_min, is_active } = req.body||{};
    const route = await prisma.route.update({ where:{ id }, data:{ distance_km, duration_min, is_active } });
    return jsonOk(res,{ route });
  }
  if(req.method==='DELETE'){
    if(!requireCsrf(req,res)) return; await prisma.route.delete({ where:{ id } }); return jsonOk(res,{ deleted:true });
  }
