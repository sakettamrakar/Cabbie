// TODO: Introduce /api/v2 when changing city payload shapes.
...
export default withApi(async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  if(req.method==='POST'){
    ...
    if(!name || !slug || !state) return sendError(res, makeError('VALIDATION_FAILED',400,'Missing fields'));
    const existing = await prisma.city.findFirst({ where:{ slug } });
    if(existing) return sendError(res, makeError('VALIDATION_FAILED',409,'Slug exists'));
