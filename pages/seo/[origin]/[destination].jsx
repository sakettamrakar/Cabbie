...
export const getStaticPaths = async () => {
    ...
    const paths = routes.map((r) => ({ params: { origin: r.origin.slug, destination: r.destination.slug } }));
    await disconnect();
    return { paths, fallback: 'blocking' };
};
export const getStaticProps = async ({ params }) => {
    const origin = params === null || params === void 0 ? void 0 : params.origin;
    const destination = params === null || params === void 0 ? void 0 : params.destination;
    const bundle = await fetchRouteBySlugs(origin, destination);
    const buildTime = new Date();
    if (!bundle) {
        await disconnect();
        return { props: { origin, destination, distance: 0, duration: 0, fares: [], faqs: [], highlights: [], intro: '', pickupPoints: [], dropPoints: [], words: '', ctas: [], routeId: 0, inactive: true, updatedOn: buildTime.toISOString() }, revalidate: 3600 };
    }
    const { route } = bundle;
    ...
    await disconnect();
    const fares = route.fares.sort((a, b) => a.base_fare_inr - b.base_fare_inr).map((f) => ({ car_type: f.car_type, base: f.base_fare_inr }));
    ...
    while (words.split(' ').length < wordCountTarget) {
        words += ' ' + baseParagraph;
    }
...
export default function RouteSeoPage({ origin, destination, distance, duration, fares, faqs, highlights, intro, pickupPoints, dropPoints, ctas, routeId, inactive, updatedOn }) {
    var _a;
    ...
    const jsonLd = [faqLd, taxiServiceSchema({ origin, destination, offers: fares.map(f => ({ name: f.car_type, priceInr: f.base })) })].filter(Boolean);
    ...
    if (process.env.ENABLE_HI_LOCALE === '1') {
        try {
            ...
            alternates.push({ href: SITE_BASE_URL + routePath('hi', pathName), hrefLang: 'hi-IN' });
        }
        catch { }
    }
    ...
    const blocks = [];
    let acc = [];
    const WORDS_PER_BLOCK = 120;
    words.forEach((w) => { acc.push(w); if (acc.length >= WORDS_PER_BLOCK) {
  ...
  <p style={{ fontSize: 12, marginTop: 4, color: '#555' }}>{translate('updated_on')} {new Date(updatedOn).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}</p>
    ...
    {(highlights === null || highlights === void 0 ? void 0 : highlights.length) > 0 && <section aria-labelledby="whyHeading" style={{ marginTop: 32 }}><h2 id="whyHeading">Why Choose Us</h2><ul>{highlights.map((h, i) => <li key={i}>{h}</li>)}</ul></section>}
    <section id="quickBooking" style={{ marginTop: 40 }}>
      <details>
        <summary style={{ cursor: 'pointer' }}>Quick Booking Form</summary>
        <div style={{ marginTop: 16 }}>
          <BookingIsland routeId={routeId} preselectedCarType={((_a = fares[0]) === null || _a === void 0 ? void 0 : _a.car_type) || 'HATCHBACK'} defaultOrigin={origin} defaultDestination={destination} onBooked={(id) => { console.log('Booked', id); }}/>
        </div>
      </details>
    </section>
    <FaqList faqs={faqs}/>
