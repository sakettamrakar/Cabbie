...
interface FareRow { id:number; car_type:string; base_fare_inr:number; night_surcharge_pct:number; updated_at:string; route:{ id:number; origin:City; destination:City }; }
...
export default function AdminFares(){
  ...
  const [bulkMode,setBulkMode]=useState<'PCT'|'DELTA'|'SET'>('PCT');
  ...
  function startEdit(row:FareRow){
    setEditId(row.id); setEditBase(row.base_fare_inr.toString()); setEditNight(row.night_surcharge_pct.toString());
  }
  ...
  const res = await adminFetch('/api/admin/fares/bulk',{method:'POST',body:JSON.stringify({ car_type:bulkCar, mode:bulkMode, value:Number(bulkValue) })});
    const data = await res.json();
    if(!data.ok){ setBulkResult(data.error||'Bulk update failed'); return; }
