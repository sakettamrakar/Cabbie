...
interface RouteReport { route_id:number; origin:string; destination:string; totalBookings:number; totalRevenue:number; codBookings:number; onlineBookings:number; }

export default function AdminReports(){
  const [data,setData]=useState<RouteReport[]>([]);
  const [top,setTop]=useState<RouteReport[]>([]);
  ...
  async function load(){
    ...
    try {
      ...
      setData(j.routes); setTop(j.top5);
