...
export default function AdminOffers() {
    ...
    const [form, setForm] = useState({ code: '', discount_type: 'FLAT', value: '', cap_inr: '', valid_from: '', valid_to: '', active: true, conditions: '', title: '', description: '' });
    ...
    useEffect(() => { load(); }, []);
    function startEdit(o) { var _a; setForm({ id: o.id, code: o.code, discount_type: o.discount_type, value: o.value.toString(), cap_inr: ((_a = o.cap_inr) === null || _a === void 0 ? void 0 : _a.toString()) || '', valid_from: o.valid_from ? o.valid_from.substring(0, 10) : '', valid_to: o.valid_to ? o.valid_to.substring(0, 10) : '', active: o.active, conditions: o.conditions ? o.conditions : '', title: o.title || '', description: o.description || '' }); }
    ...
    async function submit(e) {
        ...
        if (!form.code || !form.value)
            return;
        const payload = { code: form.code.toUpperCase(), discount_type: form.discount_type, value: Number(form.value), cap_inr: form.cap_inr ? Number(form.cap_inr) : null, valid_from: form.valid_from || null, valid_to: form.valid_to || null, active: form.active, conditions: form.conditions ? safeJson(form.conditions) : null, title: form.title || null, description: form.description || null };
    ...
    async function del(id) { if (!confirm('Delete offer?'))
        return; const r = await adminFetch(`/api/admin/offers/${id}`, { method: 'DELETE' }); const j = await r.json(); if (!j.ok)
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {offers.map(o => {
              ...
              <td style={td}>{(_a = o.cap_inr) !== null && _a !== void 0 ? _a : '—'}</td>
