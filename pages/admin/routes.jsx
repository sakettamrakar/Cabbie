...
export default function AdminRoutes() {
    ...
    const [form, setForm] = useState({ origin_city_id: '', destination_city_id: '', distance_km: '', duration_min: '', is_active: true });
    ...
    async function load() {
        ...
        try {
            ...
            if (!rRes.ok)
                throw new Error(rRes.error || 'Failed routes');
            if (!cRes.ok)
                throw new Error(cRes.error || 'Failed cities');
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        if (form.origin_city_id === '' || form.destination_city_id === '')
            return;
        const payload = { origin_city_id: Number(form.origin_city_id), destination_city_id: Number(form.destination_city_id), distance_km: form.distance_km ? Number(form.distance_km) : null, duration_min: form.duration_min ? Number(form.duration_min) : null, is_active: form.is_active };
    ...
    function startEdit(r) {
        var _a, _b;
        setForm({ id: r.id, origin_city_id: r.origin_city_id, destination_city_id: r.destination_city_id, distance_km: ((_a = r.distance_km) === null || _a === void 0 ? void 0 : _a.toString()) || '', duration_min: ((_b = r.duration_min) === null || _b === void 0 ? void 0 : _b.toString()) || '', is_active: r.is_active });
    }
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {routes.map(r => {
            ...
            <td style={td}>{(_a = r.distance_km) !== null && _a !== void 0 ? _a : '—'}</td>
            <td style={td}>{(_b = r.duration_min) !== null && _b !== void 0 ? _b : '—'}</td>
