...
export default function AdminBookings() {
    ...
    const [filters, setFilters] = useState({ id: '', phone: '', date: '', status: '' });
    async function load() {
        setLoading(true);
        setError(null);
        try {
            ...
            if (!bRes.ok)
                throw new Error(bRes.error || 'Bookings fetch failed');
            if (!dRes.ok)
                throw new Error(dRes.error || 'Drivers fetch failed');
            setBookings(bRes.bookings);
            setDrivers(dRes.drivers.filter((d) => d.active));
        }
        catch (e) {
            setError(e.message);
        }
    ...
    function applyFilters(e) { e.preventDefault(); load(); }
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {bookings.map(b => {
                ...
                const assigned = (_b = (_a = b.assignments[0]) === null || _a === void 0 ? void 0 : _a.driver) === null || _b === void 0 ? void 0 : _b.name;
                return <tr key={b.id}>
              <td style={td}>{b.id}</td>
              <td style={td}>{(_d = (_c = b.route) === null || _c === void 0 ? void 0 : _c.origin) === null || _d === void 0 ? void 0 : _d.slug} → {(_f = (_e = b.route) === null || _e === void 0 ? void 0 : _e.destination) === null || _f === void 0 ? void 0 : _f.slug}</td>
              <td style={td}>{new Date(b.pickup_datetime).toLocaleString()}</td>
              <td style={td}>{b.car_type}</td>
              <td style={td}>{b.customer_phone}</td>
              <td style={td}>{b.status}</td>
              <td style={td}>
                {b.status === 'COMPLETED' || b.status === 'CANCELLED' ? '—' : <select defaultValue="" onChange={e => { const val = e.target.value; if (val) {
                        assignDriver(b.id, Number(val));
                        e.target.value = '';
                    } }}>
...
const filterInput = { padding: '6px 8px', border: '1px solid #cbd5e1', borderRadius: 4, fontSize: 13 };
