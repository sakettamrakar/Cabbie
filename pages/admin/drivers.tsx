...
interface Driver { id:number; name:string; phone:string; car_type:string; vehicle_no?:string|null; active:boolean; }

export default function AdminDrivers(){
  const [drivers,setDrivers]=useState<Driver[]>([]);
  const [loading,setLoading]=useState(true);
  const [error,setError]=useState<string|null>(null);
  const [form,setForm]=useState<{id?:number;name:string;phone:string;car_type:string;vehicle_no:string;active:boolean}>({name:'',phone:'',car_type:'SEDAN',vehicle_no:'',active:true});
  const editing = form.id!==undefined;

  async function load(){
    setLoading(true); setError(null);
    try { const r=await fetch('/api/admin/drivers'); const j=await r.json(); if(!j.ok) throw new Error(j.error||'Fetch failed'); setDrivers(j.drivers);} catch(e:any){ setError(e.message);} finally{ setLoading(false);} }
  useEffect(()=>{ load(); },[]);

  function startEdit(d:Driver){ setForm({ id:d.id, name:d.name, phone:d.phone, car_type:d.car_type, vehicle_no:d.vehicle_no||'', active:d.active }); }
  ...
  async function del(id:number){ if(!confirm('Delete driver?')) return; const r=await adminFetch(`/api/admin/drivers/${id}`,{method:'DELETE'}); const j=await r.json(); if(!j.ok) alert(j.error||'Delete failed'); else load(); }
