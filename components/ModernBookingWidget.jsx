...
export default function ModernBookingWidget({ className = '' }) {
    ...
    return (<section className={`booking-widget card ${className}`} aria-label={`${SITE_BRAND} booking widget`}>
      ...
      <div className="booking-widget__tabs" role="tablist" aria-label="Trip type">
        ...
        ].map((tab) => (<button key={tab.id} type="button" role="tab" aria-selected={tripType === tab.id} className={`booking-widget__tab ${tripType === tab.id ? 'is-active' : ''}`} onClick={() => setTripType(tab.id)}>
            {tab.label}
          </button>))}
      </div>
      ...
      <form className="booking-widget__form" onSubmit={handleSubmit} noValidate>
        <div className="form-row grid-2">
          <div className="form-field">
            ...
            <div className="form-control with-icon">
              ...
              <input id="origin" name="origin" type="text" autoComplete="off" placeholder="e.g. Raipur Airport" value={formData.origin} onChange={handleInputChange} required/>
            </div>
          </div>
          <div className="form-field">
            ...
            <div className="form-control with-icon">
              ...
              <input id="destination" name="destination" type="text" autoComplete="off" placeholder="e.g. Bilaspur Railway Station" value={formData.destination} onChange={handleInputChange} required/>
            </div>
          </div>
        </div>
        ...
        <div className="form-row grid-2">
          <div className="form-field">
            ...
            <input id="pickupDate" name="pickupDate" type="date" value={formData.pickupDate} onChange={handleInputChange} min={todayISO()} required/>
          </div>
          <div className="form-field">
            <label htmlFor="pickupTime">Pickup time</label>
            <input id="pickupTime" name="pickupTime" type="time" value={formData.pickupTime} onChange={handleInputChange} required/>
          </div>
        </div>
        ...
        {errorMessage && (<p className="form-error" role="alert" aria-live="polite">
            {errorMessage}
          </p>)}
