...
export interface BookingFormProps {
  routeId: number;
  preselectedCarType?: 'HATCHBACK' | 'SEDAN' | 'SUV';
  onBooked?: (bookingId:number)=>void;
  defaultOrigin?: string;
  defaultDestination?: string;
}

export function BookingForm({ routeId, preselectedCarType='HATCHBACK', onBooked, defaultOrigin='', defaultDestination='' }: BookingFormProps){
  ...
  const [pickupDt, setPickupDt] = useState<string>('');
  ...
  const [validOffers,setValidOffers]=useState<string[]>([]);
  const [offerStatus,setOfferStatus]=useState<string>('');
  const [quote,setQuote]=useState<{fare_quote_inr:number;distance_km:number;duration_min:number}|null>(null);
  const [farePreview,setFarePreview]=useState<number|null>(null);
  const [offersVersion,setOffersVersion]=useState<number|undefined>();
  ...
  const loadOffers = useMemo(()=> debounce(async () => {
    ...
    const activeOffers = (j.offers as any[]).filter(o=> o.active && (!o.valid_from || new Date(o.valid_from).getTime()<=now) && (!o.valid_to || new Date(o.valid_to).getTime()>=now));
        ...
        (window as any).__activeOffers = activeOffers;
      }
    }catch{}
  }, 150), []);
  useEffect(()=>{ if(validOffers.length===0) loadOffers(); },[validOffers.length, loadOffers]);
  ...
  useEffect(()=>{
    try {
      ...
      if(raw){
        ...
        if(data.originText) setOriginText(data.originText);
        if(data.destinationText) setDestinationText(data.destinationText);
        if(data.pickupDt) setPickupDt(data.pickupDt);
        if(data.customerPhone) setCustomerPhone(data.customerPhone);
        if(data.customerName) setCustomerName(data.customerName);
        if(data.discountCode) { setDiscountCode(data.discountCode); scheduleMicrotask(()=>recalcDiscountPreview(data.discountCode)); }
      }
    }catch{}
  ...
  const computeQuote = useMemo(()=> debounce(async ()=>{
    ...
    try {
      ...
      const j = await r.json();
      if(j.ok){ setQuote(j.data); setFarePreview(j.data.fare_quote_inr); }
    }catch{}
  }, 250), [originText, destinationText, carType]);
  useEffect(()=>{ if(originText && destinationText) computeQuote(); },[originText,destinationText,computeQuote]);

  function recalcDiscountPreview(code:string){
    if(!quote){ setFarePreview(null); return; }
    if(!code || !validOffers.includes(code.toUpperCase())){ setFarePreview(quote.fare_quote_inr); return; }
    const list = (window as any).__activeOffers || [];
    const offer = list.find((o:any)=> o.code.toUpperCase()===code.toUpperCase());
    if(!offer){ setFarePreview(quote.fare_quote_inr); return; }
    let discounted = quote.fare_quote_inr;
    if(offer.discount_type==='FLAT') discounted = Math.max(0, discounted - offer.value);
    ...
    if(offer.cap_inr!=null && offer.discount_type==='PCT'){
      ...
      if(applied > maxDiscount) discounted = quote.fare_quote_inr - maxDiscount;
    }
    setFarePreview(discounted);
  }

  async function sendOtp(){
    if(!/^[0-9]{10}$/.test(customerPhone)){ setOtpMessage('Enter valid 10 digit phone'); setOtpStatus('error'); return; }
    try {
      setOtpStatus('idle'); setOtpMessage(''); setCanResend(false);
      ...
      const j = await r.json();
      if(j.ok){ setOtpStatus('sent'); setOtpMessage(j.otp ? `OTP sent (DEV: ${j.otp})` : 'OTP sent'); }
      else { setOtpStatus('error'); setOtpMessage(j.error||'OTP failed'); }
  if(j.ok){ track('otp_sent', attachUtm({ origin: originText, destination: destinationText, phone: customerPhone })); }
    } catch(e:any){ setOtpStatus('error'); setOtpMessage(e.message||'OTP error'); }
    finally {
      // Enable resend after 30s if not yet verified
      setTimeout(()=>{ if(otpStatus!=='verified') setCanResend(true); },30000);
    }
  }
  ...
  async function verify(){
    ...
    try {
      ...
      const j = await r.json();
  if(j.ok && j.verified){ setOtpStatus('verified'); setOtpMessage('OTP verified'); track('otp_verified', attachUtm({ origin: originText, destination: destinationText })); }
      else { setOtpStatus('error'); setOtpMessage(j.error||'Invalid OTP'); }
    } catch(e:any){ setOtpStatus('error'); setOtpMessage(e.message||'Verify error'); }
  }
  ...
  discount_code: discountCode && validOffers.includes(discountCode.toUpperCase())? discountCode.toUpperCase(): undefined
      ...
      const j = await r.json();
  if(j.ok){ setOtpMessage(`Booking confirmed. ID: ${j.booking.id}`); const eventId = j.booking.event_id; track('booking_created', attachUtm({ origin: originText, destination: destinationText, car_type: carType, fare: j.booking.fare_quote_inr || quote?.fare_quote_inr, booking_id: String(j.booking.id), payment_mode:'COD', event_id: eventId } as any)); if(onBooked) onBooked(j.booking.id); }
      else { setOtpStatus('error'); setOtpMessage(j.error||'Booking failed – please retry.'); }
    } catch(err:any){ setOtpStatus('error'); setOtpMessage(err.message||'Network error – booking not saved.'); }
    finally { setSubmitting(false); }
  }

  const errors: Record<string,string> = {};
  if(otpStatus==='error' && otpMessage) errors['otp'] = otpMessage;
  if(otpStatus==='error' && /Enter valid/.test(otpMessage)) errors['customer_phone']=otpMessage;
  return (
    <form aria-labelledby="bookingHeading" noValidate onSubmit={submit} style={{display:'grid', gap:10, maxWidth:400}}>
  ...
  {errors.customer_phone && <div id="phone_error" role="alert" aria-live="assertive" style={{color:'#b00',fontSize:12,marginTop:4}}>{errors.customer_phone}</div>}
      ...
      <div>
        ...
        <input id="discount_code" name="discount_code" value={discountCode} onChange={e=>{ const v=e.target.value.trim(); setDiscountCode(v); scheduleMicrotask(()=>{ if(!v){ setOfferStatus(''); recalcDiscountPreview(''); } else { const ok=validOffers.includes(v.toUpperCase()); setOfferStatus(ok?'Applied':'Invalid'); recalcDiscountPreview(v); } }); }} placeholder="Enter code" type="text" aria-describedby="discountHelp" />
