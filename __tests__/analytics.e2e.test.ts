...
jest.mock('../lib/analytics/mp', () => ({ sendServerEvent: jest.fn(async (_:any)=>{}) }));
...
describe('analytics booking flow', () => {
  let otp:string; let phone='1234567890';
  test('otp send + verify + booking emits server MP event without PII', async () => {
    ...
    const calls = (sendServerEvent as jest.Mock).mock.calls;
    expect(calls.length).toBeGreaterThan(0);
    const evt = calls.find(c=>c[0].name==='booking_created')?.[0];
