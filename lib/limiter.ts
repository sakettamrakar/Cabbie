...
interface RateOptions { key:string; windowSec:number; max:number }
...
export async function rateLimit(opts:RateOptions){
  const now = Date.now();
  const r = getRedis();
  if(r){
    ...
    if(!rec || rec.exp < now){ mem.set(opts.key,{ c:1, exp: now + opts.windowSec*1000 }); return { allowed:true, remaining: opts.max-1 }; }
    rec.c++; return { allowed: rec.c <= opts.max, remaining: Math.max(0, opts.max - rec.c) };
  }
}
