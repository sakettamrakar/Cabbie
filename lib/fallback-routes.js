 ...
 */
export function generateFallbackRouteData(origin, destination) {
 ...
 */
function getDistanceFromMatrix(origin, destination) {
    const routes = {
        // Chhattisgarh routes
...
function calculateDuration(distance) {
    if (distance <= 50)
        return Math.round(distance * 1.5); // ~40 km/h
    if (distance <= 200)
        return Math.round(distance * 1.2); // ~50 km/h  
    if (distance <= 500)
        return Math.round(distance * 1.0); // ~60 km/h
...
function generateHighlights(origin, destination, distance) {
    const base = [
...
function generateFAQs(origin, destination, distance, fare) {
...
function generateRelatedRoutes(origin, destination) {
