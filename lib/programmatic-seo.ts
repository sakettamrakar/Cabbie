 ...
 */
export function getAllAvailableRoutes(): RouteData[] {
 ...
 */
export function generateSEOForRoute(origin: string, destination: string): SEORouteInfo | null {
...
function calculateDuration(distance: number): number {
  if (distance <= 50) return distance * 1.5; // City/local roads: ~40 km/h
  if (distance <= 200) return distance * 1.2; // State highways: ~50 km/h  
  if (distance <= 500) return distance * 1.0; // National highways: ~60 km/h
 ...
 */
function calculateEstimatedFare(distance: number): number {
 ...
 */
function getRouteFeatures(distance: number): string[] {
 ...
 */
export function generateAllRouteSlugs(): { origin: string; destination: string }[] {
  const routes = getAllAvailableRoutes();
 ...
 */
export function routeExists(origin: string, destination: string): boolean {
  const routes = getAllAvailableRoutes();
 ...
 */
export function getRelatedRoutes(origin: string, destination: string, limit: number = 5): RouteData[] {
  const routes = getAllAvailableRoutes();
