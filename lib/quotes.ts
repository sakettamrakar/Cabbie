...
export interface QuoteInput { origin_text: string; destination_text: string; car_type: string; }
export interface QuoteResult { fare_quote_inr: number; distance_km: number; duration_min: number; }

// Mock distance/duration matrix (km, minutes)
// NOTE: Add both directions for asymmetric lookups.
const ROUTE_MATRIX: Record<string, { d: number; t: number }> = {
...
export function getQuote({ origin_text, destination_text, car_type }: QuoteInput): QuoteResult {
  const o = norm(origin_text); const d = norm(destination_text);
  const key = `${o}|${d}`;
