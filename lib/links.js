...
export async function relatedRoutes(origin, destination) {
    ...
    const out = [];
    if (reverse)
        out.push({ origin: reverse.origin.slug, destination: reverse.destination.slug, distance_km: reverse.distance_km });
    neighbors.forEach(n => out.push({ origin: n.origin.slug, destination: n.destination.slug, distance_km: n.distance_km }));
    return out;
}
export async function cityTopRoutes(citySlug, limit = 12) {
