...
interface QuoteArgs { origin_text:string; destination_text:string; pickup_datetime:string|Date; car_type:string; discount_code?:string|null }
interface QuoteOut { route_id:number|null; distance_km:number; duration_min:number; fare_base_inr:number; fare_after_discount_inr:number; applied_discount?:{ code:string; amount_inr:number }|null; night_surcharge_applied:boolean; discount_code_attempted?:string|null; discount_valid?:boolean }

export async function getPricingQuote(args:QuoteArgs): Promise<QuoteOut> {
    ...
    if(offer){
      ...
      if(within){
        if(offer.discount_type==='FLAT'){
          ...
          fare -= amt; applied = { code:offer.code, amount_inr: amt }; discount_valid = true;
        } else { // PCT
          const pct = Math.round(fare * (offer.value/100));
          const capped = offer.cap_inr? Math.min(pct, offer.cap_inr): pct;
            fare -= capped; applied = { code:offer.code, amount_inr: capped }; discount_valid = true;
        }
      } else { discount_valid = false; }
    } else { discount_valid = false; }
  }
  ...
  if(hr >=22 || hr <5){
    fare = Math.round(fare*1.2); night=true;
  }
