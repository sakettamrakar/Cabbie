...
export function buildMetaDescription({ origin, destination, price, benefits, distance, duration }) {
    ensure(origin, 'origin');
    ensure(destination, 'destination');
    const intro = price
    ...
    const detailBits = [];
    if (distance)
        detailBits.push(`${distance}km`);
    if (duration)
        detailBits.push(`${Math.floor(duration / 60)}h journey`);
