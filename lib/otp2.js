...
function h(code) { return crypto.createHash('sha256').update(code).digest('hex'); }
export async function issueOTP(phone) {
    const code = gen();
    const rec = { hash: h(code), exp: Date.now() + TTL_MS };
    const r = getRedis();
    if (r) {
        await r.set(PREFIX + phone, JSON.stringify(rec), 'PX', TTL_MS);
    }
    else {
        mem.set(phone, rec);
    }
    ...
    if (dev)
        return { phone, otp: code, expires_in: TTL_MS / 1000 };
